...
class A2AMessage:
    ...
    content: str
    name: Optional[str] = None  # Optional name for the message
    tool_calls: Optional[List[Dict[str, Any]]] = None  # Tool calls if role is "assistant"
    tool_call_id: Optional[str] = None  # ID of tool call result if role is "tool"
...
class A2AClient:
    ...
    async def invoke_agent(self, task: Union[A2ATask, str], 
                          data: Optional[Dict[str, Any]] = None) -> A2AResponse:
        """
...
class A2ACoordinator:
    ...
    async def distribute_task(self, task: A2ATask, 
                             agent_filter: Optional[List[str]] = None) -> Dict[str, A2AResponse]:
        """
