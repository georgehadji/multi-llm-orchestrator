...
def list_sessions():
    return {
        "sessions": [
            {"id": sid, "project_name": s.get("project_name", "Untitled")}
...
Starting now..."""
                ...
                if session_id in sessions:
                    sessions[session_id]["messages"].append(
                        {"role": "assistant", "content": response_text, "thinking": False}
                    )

            elif event == "session_update":
                logger.info(f"Session update: {payload}")
                # Acknowledge update
                await websocket.send_json({"event": "session_state", "data": {"settings": payload}})
