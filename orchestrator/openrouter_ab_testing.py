...
class OpenRouterABTester:
    ...
    def should_use_optimization(
        ...
        """
        opt_type = (
            optimization if isinstance(optimization, OptimizationType)
    ...
    def analyze_experiment(
        ...
        if control_samples > 0 and treatment_samples > 0:
            ...
            if result.control_error_rate == 0:
                ...
                result.error_rate_improvement = 0.0 if result.treatment_error_rate == 0 else float('-inf')
            else:
            ...
            result.cost_improvement = (
                (result.treatment_avg_cost - result.control_avg_cost)
                / max(result.control_avg_cost, 0.001)
            )
            
            # Simple adoption criteria (can be made more sophisticated)
            has_enough_samples = (
                control_samples >= self.MIN_SAMPLES 
                and treatment_samples >= self.MIN_SAMPLES
            )
            ...
            result.should_adopt = has_enough_samples and error_rate_better and latency_acceptable and cost_acceptable
    ...
    def clear_experiment(self, optimization: OptimizationType | str) -> None:
        ...
        with sqlite3.connect(str(self.db_path)) as conn:
            conn.execute(
                "DELETE FROM ab_test_records WHERE optimization = ?",
                (opt_type.value,)
            )
            conn.commit()
