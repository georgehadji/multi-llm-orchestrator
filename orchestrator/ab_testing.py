...
class ABTestingEngine:
    ...
    async def analyze_results(self, experiment_id: str) -> Optional[ExperimentResult]:
        """
...
class SequentialABTest:
    ...
    def __init__(self, config: Optional[SequentialTestConfig] = None):
...
class CUPEDAdjustment:
    ...
    def adjust_metrics(
        ...
        adjusted_treatment = [
            ...
            for y, x in zip(treatment, pre_experiment)
        ]
        
        adjusted_control = [
            y - self.theta * (x - mean_pre)
            for y, x in zip(control, pre_experiment)
        ]
    ...
    def _estimate_theta(
        ...
        # Covariance
        cov = sum((y - mean_y) * (x - mean_x) for y, x in zip(metrics, covariate)) / (n - 1)
