...
class ProgressRenderer:
    ...
    def handle(self, event: Any) -> None:
        if isinstance(event, ProjectStarted):
            ...
            total_tasks = getattr(event, 'total_tasks', None) or event.metadata.get('total_tasks', 0)
            budget_usd = getattr(event, 'budget_usd', None) or getattr(event, 'budget', None) or event.metadata.get('budget', 0.0)
            self.total = total_tasks
            if not self.quiet:
                print(
                    f"\n▶  Project started — {total_tasks} tasks  "
            ...
            task_type = getattr(event, 'task_type', None) or event.metadata.get('task_type', 'unknown')
            ...
            if not self.quiet:
                ...
                if best_score is None:
                    best_score = event.metadata.get('score', score) if hasattr(event, 'metadata') else score
                print(
            ...
            if status is None:
                ...
                from .models import TaskStatus
                status = TaskStatus(status_str) if status_str in ['completed', 'failed', 'degraded'] else TaskStatus.COMPLETED
            self.completed += 1
            ...
            # Handle both unified_events (metadata) and events.py (direct attribute)
            task_id = getattr(event, 'task_id', None) or event.metadata.get('task_id', 'unknown')
            reason = getattr(event, 'reason', None) or getattr(event, 'error', None) or event.metadata.get('reason') or event.metadata.get('error', 'Unknown error')
            self.failed += 1
            self._active.pop(task_id, None)
            if not self.quiet:
                print(
            ...
            spent_usd = getattr(event, 'spent_usd', None) or getattr(event, 'spent', None) or event.metadata.get('spent', 0.0)
            cap_usd = getattr(event, 'cap_usd', None) or getattr(event, 'budget', None) or event.metadata.get('budget', 0.0)
            ...
            total_cost_usd = getattr(event, 'total_cost_usd', None) or getattr(event, 'total_cost', None) or event.metadata.get('total_cost', 0.0)
            elapsed_seconds = getattr(event, 'elapsed_seconds', None) or getattr(event, 'duration_seconds', None) or event.metadata.get('duration_seconds', 0.0)
            tasks_completed = getattr(event, 'tasks_completed', None) or event.metadata.get('tasks_completed', 0)
            tasks_failed = getattr(event, 'tasks_failed', None) or event.metadata.get('tasks_failed', 0)
            if not self.quiet:
