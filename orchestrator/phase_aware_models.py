...
class PhaseAwareModelSelector:
    ...
    def select_model(
        ...
        for model in available_models:
            ...
            cap_scores = [ModelCapabilities.get_score(model, cap) for cap in required_caps]
...
def example():
    ...
    for config_name, config in configs.items():
        total_input = sum(
            MODEL_COSTS.get(model, {"input": 0})["input"] for model in config.values()
        )
        total_output = sum(
            MODEL_COSTS.get(model, {"output": 0})["output"] for model in config.values()
        )
