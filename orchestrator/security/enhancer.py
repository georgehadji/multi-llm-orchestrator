...
def security_system_prompt() -> str:
    ...
    for cat in ["web", "api", "auth", "data", "infra"]:
        cat_name = {"web": "Web Security", "api": "API Security", "auth": "Authentication",
                     "data": "Data Security", "infra": "Infrastructure"}[cat]
        rules = [r for r in SECURITY_RULES if r.category == cat and r.severity == "critical"]
        rules += [r for r in SECURITY_RULES if r.category == cat and r.severity != "critical"]
        if rules:
            lines.append(f"### {cat_name}")
            for r in rules:
                prefix = {"critical": "[CRITICAL]", "high": "[HIGH]", "medium": "[MEDIUM]"}[r.severity]
...
class OpenGraphGenerator:
    ...
    def generate_head_tags(self, title: str, description: str, url: str,
                            image_url: str = "", site_name: str = "",
    ...
    def generate_json_ld(self, name: str, description: str, url: str,
                          logo_url: str = "") -> str:
    ...
    def full_head_section(self, title: str, description: str, url: str,
                           image_url: str = "", site_name: str = "",
                           og_type: str = "website") -> str:
