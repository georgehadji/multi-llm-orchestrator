    ...
    def decorator(func: Callable[..., Any]) -> Callable[..., Any]:
        ...
        async def wrapper(*args: Any, **kwargs: Any) -> Any:
            ...
            wait_strategy = wait_exponential(
                multiplier=exponential_base, min=min_wait, max=max_wait
            )

            if jitter:
                # Add random jitter (±20% of wait time)
                wait_strategy = wait_strategy + wait_random(min=min_wait * 0.2, max=max_wait * 0.2)
            ...
            retrying = AsyncRetrying(
                ...
                after=after_log(logging.getLogger(logger_name), logging.INFO),
            )
        ...
        for attempt in range(1, max_attempts + 1):
            try:
                ...
                if attempt == max_attempts:
                    logger.error(f"Failed after {max_attempts} attempts: {e}", exc_info=True)
                    raise

                # Calculate wait time with optional jitter
                actual_wait = wait_time
                if jitter:
                    actual_wait += random.uniform(-wait_time * 0.2, wait_time * 0.2)
...
cache_retry = async_retry(
    max_attempts=3, timeout=15, min_wait=0.2, max_wait=2.0, retryable_exceptions=(RetryableError,)
)
...
class RetryStrategy:
    ...
    def create_decorator(
        self, retryable_exceptions: tuple[Type[Exception], ...] = (RetryableError,)
    ) -> Callable:
    ...
    def aggressive(cls) -> "RetryStrategy":
        ...
        return cls(max_attempts=5, timeout=300, min_wait=2.0, max_wait=60.0, exponential_base=2.0)

    @classmethod
    def conservative(cls) -> "RetryStrategy":
        """Conservative retry strategy (fewer attempts, shorter waits)"""
        return cls(max_attempts=2, timeout=60, min_wait=0.5, max_wait=10.0, exponential_base=1.5)

    @classmethod
    def fast_fail(cls) -> "RetryStrategy":
        """Fast-fail strategy (minimal retries for latency-critical ops)"""
        return cls(max_attempts=1, timeout=30, min_wait=0.1, max_wait=1.0, jitter=False)
...
def should_retry(
    exc: Exception, attempt: int, max_attempts: int, elapsed_time: float, timeout: float
) -> bool:
    """
...
if __name__ == "__main__":
    ...
    @custom_strategy.create_decorator(retryable_exceptions=(TimeoutError, RateLimitError))
