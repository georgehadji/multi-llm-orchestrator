...
class PluginRegistry:
    ...
    def __init__(self) -> None:
        ...
        self._hooks: dict[PluginPhase, list[Plugin]] = {phase: [] for phase in PluginPhase}
        self._initialized = False
    ...
    async def execute_hook(
        ...
        plugins_sorted = sorted(
            plugins, key=lambda p: p.metadata.priority.value if p.is_enabled else 999
        )
        ...
        for plugin in plugins_sorted:
            ...
            try:
                ...
                logger.error(f"Error in plugin {plugin.metadata.name} " f"hook {phase.value}: {e}")
