...
class GitIntegration:
    ...
    def __init__(self, repo_path: Path, config: GitConfig | None = None):
        self.repo_path = Path(repo_path)
        self.config = config or GitConfig()
        self._git_available = self._check_git()
        self._original_branch: str | None = None
        self._project_branch: str | None = None
...
Generated by Multi-LLM Orchestrator"""
    ...
    def get_commit_history(self, limit: int = 10) -> list[dict[str, Any]]:
        ...
        try:
            ...
            result = self._run_git(["log", f"-{limit}", f"--format={format_str}"], check=False)
...
def get_default_git_config() -> GitConfig:
    ...
    return GitConfig(
        ...
        strategy=CommitStrategy(os.environ.get("ORCHESTRATOR_GIT_STRATEGY", "after_each_task")),
