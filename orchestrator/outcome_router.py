...
from .models import Model, TaskType, Task, ROUTING_TABLE, COST_TABLE
from .feedback_loop import FeedbackLoop, CodebaseFingerprint, ProductionOutcome
from .leaderboard import ModelLeaderboard, get_leaderboard
from .plugins import get_plugin_registry, RouterPlugin, RoutingSuggestion
...
class RoutingContext:
    ...
    budget_total: float
    codebase_fingerprint: Optional[CodebaseFingerprint] = None
    strategy: RoutingStrategy = RoutingStrategy.BALANCED
    previous_model: Optional[Model] = None  # For fallback
...
class OutcomeWeightedRouter:
    ...
    def _select_exploration_candidate(
        ...
        cumulative = 0
        for (model, _), weight in zip(candidates, weights):
            cumulative += weight
            if r <= cumulative:
                return model
        ...
        return {
            ...
            "exploration_status": {
                ...
                "underexplored_models": [
                    ...
                    if m not in {k[0] for k in self.feedback._performance_records.keys()}
                ],
            },
        }
...
_router: Optional[OutcomeWeightedRouter] = None
