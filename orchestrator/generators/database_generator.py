...
class DatabaseSchemaGenerator:
    ...
    async def from_description(
        ...
        logger.info(f"Generated schema with {len(schema.tables)} tables for {db_type.value}")
...
'''
    ...
    def _generate_sqlalchemy_model(self, table: Table) -> str:
        ...
        for column in table.columns:
            ...
            if (
                column.default
                and column.default != "NOW()"
                and column.default != "gen_random_uuid()"
            ):
...
"""
    ...
    def _generate_prisma_model(self, table: Table) -> str:
        ...
        for column in table.columns:
            ...
            if column.foreign_key:
                column.foreign_key.split(".")[0].title().replace("_", "")
                code += " @relation(references: [id])"
...
_default_generator: DatabaseSchemaGenerator | None = None
