...
class DependencyResolver:
    ...
    def __init__(
        ...
        Args:
            ...
            context_compressor: Optional ContextCompressor instance for
                LLM-powered summarization. When None (default), falls back
                to hard truncation.
        """
    ...
    async def get_dependency_context(
        ...
        for dep_id in task.dependencies:
            ...
            context_section = await self._format_dependency_context(dep_id, dep_task, dep_result)
    ...
    async def _format_dependency_context(
        self,
        dep_id: str,
        dep_task: Task | None,
        ...
        # Truncate if exceeds limit (with optional LLM compression)
        if len(output) > self.context_truncation_limit:
            if self._compressor is not None and self._compressor.is_enabled:
                output = await self._compressor.compress(
                    output,
                    max_chars=self.context_truncation_limit,
                    dep_id=dep_id,
                )
            else:
