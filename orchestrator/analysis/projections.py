...
if TYPE_CHECKING:
    ...
    from ..unified_events.core import (
        ...
        EventBus,
        ProductionOutcomeRecordedEvent,
        TaskCompletedEvent,
        TaskFailedEvent,
    )
...
class ModelPerformanceProjection(PersistentProjection):
    ...
    async def on_production_outcome_recorded(self, event: ProductionOutcomeRecordedEvent) -> None:
        ...
        for _key, stats in self._stats.items():
    ...
    def get_leaderboard(
        ...
        if task_type:
            ...
            for (model, _tt), stats in self._stats.items():
...
def get_model_performance_projection(
    ...
    if "model_performance" not in _projections:
        # FIXED: from .unified_events.core import get_event_bus_sync
