...
class ProgressiveOutputManager:
    ...
    def _create_folder(self, phase: str, task_id: Optional[str] = None) -> Path:
        """Create numbered output folder."""
        seq = self._next_sequence()
        
        if task_id:
            folder_name = f"{seq:03d}-{task_id}"
        else:
            folder_name = f"{seq:03d}-{phase}"
    ...
    def _get_extension_for_task(self, task_type: Optional[TaskType]) -> str:
    ...
    def get_task_output_path(self, task_id: str) -> Optional[Path]:
