    ...
    async def debug_and_fix(self, test_results: list[TestResult] | None = None) -> DebugReport:
        ...
        try:
            ...
            for iteration in range(1, self.max_iterations + 1):
                ...
                iteration_result = await self._run_debug_iteration(
                ...
                if pass_rate >= self.min_pass_rate:
                    ...
                    logger.error(f"❌ Final pass rate too low: {pass_rate:.1%} (min: {self.min_pass_rate:.0%})")
        ...
        if "ERROR collecting" in full_output or "ImportError while importing" in full_output:
            ...
            if collection_match:
                ...
                analyses.append(FailureAnalysis(
                    ...
                    stack_trace="",
                    root_cause="missing_import" if "No module named" in error_detail else "syntax_error",
                    suggested_fix="Fix import or syntax error in test file" if "No module named" in error_detail else "Fix syntax error in test file",
                ))
    ...
    async def _fix_syntax_error(self, file_path: Path, failures: list[FailureAnalysis]) -> dict:
        ...
        try:
            ...
            if content != original_content:
                ...
                return {"success": False, "error": "No automatic fix available for this syntax error"}
    ...
    def _generate_summary(self) -> None:
        ...
        if self.report.iterations:
            ...
            self.report.summary = {
                ...
                "success_rate": (initial_failures - final_failures) / initial_failures * 100 if initial_failures > 0 else 0,
            }
        else:
            self.report.summary = {
                "initial_failures": 0,
