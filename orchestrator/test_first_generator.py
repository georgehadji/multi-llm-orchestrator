...
Usage:
    ...
    tdd = TestFirstGenerator(client, sandbox)
...
from .models import Task, TaskResult, TaskType, Model
...
class TestFirstGenerator:
    ...
    async def generate_with_tests(
        ...
        # ═══════════════════════════════════════════════════════
        logger.info(f"  Phase 1: Generating test suite...")
        ...
        # ═══════════════════════════════════════════════════════
        logger.info(f"  Phase 2: Generating implementation...")
        ...
        # ═══════════════════════════════════════════════════════
        logger.info(f"  Phase 3: Running tests...")
    ...
    async def _generate_test_spec(
        ...
        prompt += (
            f"Requirements for tests:\n"
            f"1. Include edge cases (empty inputs, max values, error conditions)\n"
            f"2. Test error handling and exception cases\n"
            f"3. Include type checking where applicable\n"
            f"4. Cover happy path and at least 3 edge cases\n"
            f"5. Use descriptive test names (test_should_... pattern)\n"
            f"6. Include assertions for expected behavior\n\n"
            f"Output ONLY test code. Do NOT write implementation.\n"
            f"Use pytest framework. Include all necessary imports.\n\n"
            f"```python\n"
        )
    ...
    async def _generate_code_to_pass_tests(
        ...
        try:
            response = await self.client.call(
                model=model,
                prompt=prompt,
                system=f"You are an expert software engineer. "
                       f"Write clean, production-ready code that passes all tests. "
                       f"Output ONLY implementation code.",
                max_tokens=4000,
    ...
    async def _repair_to_pass_tests(
        ...
        for iteration in range(self.max_test_iterations):
            ...
            prompt += (
                f"\nRequirements:\n"
                f"1. Fix ALL test errors\n"
                f"2. Preserve working functionality\n"
                f"3. Output ONLY the fixed implementation code\n\n"
                f"```python\n"
            )
        ...
        logger.warning(f"Max repair iterations reached, returning best effort")
