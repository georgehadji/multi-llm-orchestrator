...
def exit_code_for(outcomes) -> int:
    """Worst outcome wins: 1 (build failed) beats 2 (gate rejected) beats 0.
...
def _print_summary(outcomes, report_path: Path) -> None:
    ...
    for outcome in outcomes:
        if not outcome.success:
            status = "BUILD FAILED"
        elif outcome.quality_gate_passed is False:
            status = "REJECTED"
        elif outcome.quality_gate_passed is True:
            status = "shippable"
        else:
            status = "ungated"
        print(
            f"  {outcome.slug:28s} {status:13s} "
            f"score {outcome.score:.2f}  ${outcome.cost_usd:.4f}  "
    ...
    print(
        f"\n{len(outcomes)} site(s): {shippable} shippable, {rejected} rejected, "
        f"{failed} build failure(s).  Total cost: ${total_cost:.4f}"
    )
