...
from ..infrastructure.artifact_emitters.cicd_emitter import CicdEmitter
from ..infrastructure.artifact_emitters.docker_emitter import DockerEmitter
...
security-check:
	...
	bandit -r src/
	@echo "Dependency vulnerabilities are audited automatically in CI (pip-audit, fails on any finding) — see .github/workflows/ci.yml"
...
"""
    ...
    def _generate_dockerfile(self) -> list[str]:
        """Generate Dockerfile, .dockerignore, a resolved lock file, and an
        SBOM via the shared ``DockerEmitter`` (Phase 7, P-4) — this is the
        only place that owns Dockerfile emission; ``verifier.py`` delegates
        to the same emitter.
        """
        workspace = Workspace(root=self.output_dir, framework="python")
        written = asyncio.run(DockerEmitter().emit(workspace, profile=None))
        logger.info("Generated %d Docker artifacts via DockerEmitter", len(written))
        return written

    def _generate_github_actions(self) -> list[str]:
        """Generate GitHub Actions CI/CD workflow via the shared ``CicdEmitter``
        (Phase 7, P-5) — the only place that owns workflow emission. Adds
        SHA-pinned actions, a `permissions:` default, `concurrency:`
        grouping, and security jobs that can actually fail the build.
        """
        workspace = Workspace(root=self.output_dir, framework="python")
        written = asyncio.run(CicdEmitter().emit(workspace, profile=None))
        logger.info("Generated %d CI/CD artifact(s) via CicdEmitter", len(written))
        return written
