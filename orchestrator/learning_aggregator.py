...
class TaskPerformanceRecord:
    ...
    feedback: str | None = None  # Optional feedback on the result
...
class LearningAggregator:
    ...
    async def get_learning_insights(self) -> dict[str, Any]:
        ...
        async with self._lock:
            ...
            total_task_types = len({task_type for (task_type, model) in self.performance_cache})
            total_models = len({model for (task_type, model) in self.performance_cache})
    ...
    async def import_learning_data(self, import_path: str) -> bool:
        ...
        try:
            ...
            for _key, stats_data in import_data.get("aggregated_stats", {}).items():
...
async def get_routing_recommendations_global(task_type: TaskType) -> RoutingRecommendation | None:
    """
