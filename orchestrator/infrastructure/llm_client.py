...
from ..models import Model, TaskType, TASK_PROVIDER_STRATEGIES, estimate_cost, get_provider
...
class UnifiedClient:
    ...
    async def call(
        ...
        if not bypass_cache:
            ...
            if cached:
                ...
                model_for_response = (
                    model if isinstance(model, Model) else Model(model_id.split(":")[0])
                )
    ...
    async def _dispatch(
        ...
        try:
            ...
            # BUG-API-003 FIX: Catch auth errors early — don't retry 401s
            if _is_auth_error(e):
                raise AuthenticationError(
                    "OpenRouter API authentication failed. "
                    "Your OPENROUTER_API_KEY may be invalid or expired. "
