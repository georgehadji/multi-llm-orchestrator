...
async def test_record_snapshot_inserts_row(tmp_path):
    ...
    await store.record_snapshot(
        "proj-1", Model.DEEPSEEK_CHAT, TaskType.CODE_GEN, _make_profile(call_count=5)
    )
...
async def test_record_routing_event_stores_correct_fields(tmp_path):
    ...
    await store.record_routing_event(
        "proj-1", "task-1", TaskType.CODE_GEN, _make_result(score=0.91, det_passed=True)
    )
...
async def test_load_historical_profile_returns_none_below_10_calls(tmp_path):
    ...
    for i in range(9):
        await store.record_snapshot(
            f"proj-{i}", Model.DEEPSEEK_CHAT, TaskType.CODE_GEN, _make_profile(call_count=1)
        )
...
async def test_model_rankings_sorted_by_value_score(tmp_path):
    ...
    for i in range(10):
        await store.record_snapshot(
            ...
            Model.GPT_4O,
            TaskType.CODE_GEN,
            _make_profile(model=Model.GPT_4O, quality_score=0.85, avg_cost_usd=0.010, call_count=1),
        )
...
async def test_model_rankings_confidence_levels(tmp_path):
    ...
    for i in range(50):
        await store.record_snapshot(
            f"h{i}", Model.DEEPSEEK_CHAT, TaskType.CODE_GEN, _make_profile(call_count=1)
        )
    ...
    for i in range(5):
        await store.record_snapshot(
            ...
            TaskType.CODE_GEN,
            _make_profile(
                model=Model.MINIMAX_TEXT_01, quality_score=0.91, avg_cost_usd=0.030, call_count=1
            ),
        )
    ...
    assert by_model[Model.MINIMAX_TEXT_01].confidence == "COLD"
...
async def test_model_rankings_returns_modelranking_instances(tmp_path):
    ...
    for i in range(10):
        await store.record_snapshot(
            f"p{i}", Model.DEEPSEEK_CHAT, TaskType.CODE_GEN, _make_profile(call_count=1)
        )
...
async def test_task_type_leaders_returns_best_per_type(tmp_path):
    ...
    for i in range(10):
        await store.record_snapshot(
            ...
            Model.GPT_4O,
            TaskType.CODE_GEN,
            _make_profile(model=Model.GPT_4O, quality_score=0.80, avg_cost_usd=0.010, call_count=1),
        )
    ...
    for i in range(10):
        await store.record_snapshot(
            ...
            TaskType.SUMMARIZE,
            _make_profile(
                model=Model.GEMINI_FLASH, quality_score=0.88, avg_cost_usd=0.0005, call_count=1
            ),
        )
...
async def test_recommendations_identifies_cheaper_equivalent(tmp_path):
    ...
    for i in range(50):
        await store.record_snapshot(
            ...
            Model.GPT_4O,
            TaskType.CODE_GEN,
            _make_profile(model=Model.GPT_4O, quality_score=0.90, avg_cost_usd=0.010, call_count=1),
        )
