...
from orchestrator.a2a_protocol import (
    A2AManager, AgentCard, TaskSendRequest, TaskResult,
    AgentState, TaskStatus, MessagePart, A2AMessage,
)
...
class TestA2ADeadlockPrevention:
    ...
    async def test_timeout_exactly_at_limit(self):
        ...
        assert "task_boundary" not in manager._pending_responses, Invariants.A2A_NO_ORPHANED_RESPONSES
    ...
    async def test_concurrent_tasks_same_agent(self):
        ...
        tasks = [
            manager.send_task(TaskSendRequest(
        ...
        for i in range(10):
            assert f"concurrent_{i}" not in manager._pending_responses, Invariants.A2A_NO_ORPHANED_RESPONSES
            assert f"concurrent_{i}" not in manager._response_timeouts, Invariants.A2A_NO_LEAKED_TIMEOUTS
    ...
    async def test_handler_timeout_cleanup(self):
        ...
        # INVARIANT: Tracking cleaned up
        assert "task_timeout" not in manager._pending_responses, Invariants.A2A_NO_ORPHANED_RESPONSES
    ...
    async def test_handler_exception_cleanup(self):
        ...
        # INVARIANT: Tracking cleaned up
        assert "task_exception" not in manager._pending_responses, Invariants.A2A_NO_ORPHANED_RESPONSES
