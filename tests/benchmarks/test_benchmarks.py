    ...
    def test_topological_sort_performance(self, benchmark):
        ...
        tasks = {
            f"task_{i}": type(
                "Task",
                (),
                {"id": f"task_{i}", "dependencies": [f"task_{j}" for j in range(max(0, i - 3), i)]},
            )()
            for i in range(100)
        }
...
class TestMemoryTierBenchmarks:
    ...
    def test_memory_retrieve_latency(self, benchmark, memory_manager):
        ...
        async def setup():
            for i in range(10):
                await memory_manager.store(
                    project_id="benchmark_proj", content=f"Test memory {i}", memory_type="task"
                )

        asyncio.run(setup())

        async def retrieve():
            return await memory_manager.retrieve(project_id="benchmark_proj", query="test", limit=5)
...
class TestEventBusBenchmarks:
    ...
    def test_event_publish_latency(self, benchmark, event_bus):
        ...
        event = TaskCompletedEvent(task_id="benchmark_task", status="completed")
    ...
    def test_event_throughput_100(self, benchmark, event_bus):
        ...
        async def publish_many():
            events = [
                TaskCompletedEvent(task_id=f"benchmark_task_{i}", status="completed")
                for i in range(100)
            ]
...
class TestAPIClientBenchmarks:
    ...
    def test_request_serialization(self, benchmark):
        ...
        messages = [{"role": "user", "content": "Build an API" * 100}]
    ...
    def test_response_parsing(self, benchmark):
        ...
        mock_response = {
            "choices": [{"message": {"content": "Test output" * 100}}],
