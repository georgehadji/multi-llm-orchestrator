...
class TestBUG001TaskFieldSerialization:
    ...
    def test_falsify_task_fields_serialized(self):
        ...
        assert "module_name" in task_dict, \
            "BUG-001 REGRESSION: 'module_name' not in serialized dict"
        assert "tech_context" in task_dict, \
    ...
    def test_falsify_task_fields_deserialized(self):
        ...
        # FALSIFICATION CHECKS
        assert restored.target_path == "app/dashboard/Metrics.tsx", \
        ...
        assert restored.tech_context == "Next.js 14 with App Router", \
...
class TestBUG001Integration:
    ...
    def test_full_state_roundtrip(self):
        ...
        # Serialize all tasks
        tasks_dict = {
            k: _task_to_dict(v) for k, v in original_state.tasks.items()
        }
        ...
        # Deserialize
        restored_tasks = {
            k: _task_from_dict(v) for k, v in loaded_dict.items()
        }
