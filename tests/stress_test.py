...
class TestS1QuickSmoke:
    ...
    def test_s1_single_task_completes(self, tmp_path):
        ...
        with patch.object(orch.client, "call", side_effect=mock_call):
            state = _run(
                orch.run_project("Write a hello world function", "Function returns 'Hello, world!'")
            )
        ...
        async def mock_call(model, prompt, **kwargs):
            ...
            for line in prompt.split("\n"):
                if line.startswith("Task "):
                    ...
                    break
            return _make_mock_response("def foo():\n    pass  # stub", cost=0.001)
...
class TestS4ParallelLoad:
    ...
    def test_s4_three_parallel_projects(self, tmp_path):
        ...
        for i, state in enumerate(states):
            assert not isinstance(state, Exception), f"Project {i} raised an exception: {state}"
            assert state is not None
            assert state.status in (
                ProjectStatus.SUCCESS,
                ProjectStatus.PARTIAL_SUCCESS,
        ...
        log.info(
            f"S4 passed: 3 parallel projects completed, statuses={[s.status.value for s in states]}"
        )
...
class TestS5FallbackChain:
    ...
    def test_s5_fallback_on_api_failure(self, tmp_path):
        ...
        with patch.object(orch.client, "call", side_effect=mock_call):
            state = _run(orch.run_project("Build a resilient HTTP client", "Retry on failure"))

        # All models that responded should be non-openai
        for m_val in responded_models:
            model_enum = Model(m_val)
            assert (
                get_provider(model_enum) != "openai"
...
class TestS6PolicyGate:
    ...
    def test_s6_policy_violation_error_carries_detail(self):
        ...
        log.info(
            f"S6b passed: PolicyViolationError.task_id={err.task_id}, "
...
class TestS9TelemetryDrift:
    ...
    def test_s9_quality_ema_tracks_scores(self):
        ...
        log.info(
            f"S9b passed: quality {initial_quality:.4f} -> {final_quality:.4f} after 10 perfect scores"
        )
...
class TestS10FullProjectSimulation:
    ...
    def test_s10_realistic_project_pipeline(self, tmp_path):
        ...
        n_completed = sum(1 for r in state.results.values() if r.status == TaskStatus.COMPLETED)
...
class TestS12RealApiSmoke:
    ...
    @pytest.mark.skipif(not _at_least_one_provider_available(), reason="No API keys configured")
