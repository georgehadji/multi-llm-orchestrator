...
async def test_budget_concurrent_charge():
    ...
    ), f"Budget race condition! Expected {expected_total}, got {actual_total}"
...
async def test_budget_reservation_failure_recovery():
    ...
    assert (
        budget._reserved_usd == 10.0
    ), f"Reservation should be restored to $10, got ${budget._reserved_usd}"
...
async def test_router_concurrent_read_write():
    ...
    ), f"Model should be DEGRADED after 100 timeouts, got {state.value}"
...
async def test_budget_extreme_concurrency():
    ...
    assert (
        abs(budget.spent_usd - expected_spent) < 0.01
    ), f"Expected ${expected_spent:.2f}, got ${budget.spent_usd:.2f}"
