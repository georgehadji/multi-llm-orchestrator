...
def test_cli_build_subcommand_exists():
    ...
    assert (
        hasattr(cli_module, "build")
        or hasattr(cli_module, "cmd_build")
        or "build" in getattr(cli_module, "__all__", [])
        or _has_build_parser(cli_module)
    )
...
def _has_build_parser(cli_module) -> bool:
    ...
    return "build" in source and (
        "subparser" in source or "add_parser" in source or "command" in source
    )
...
def test_cmd_build_passes_docker_flag(tmp_path):
    ...
    assert (
        call_kwargs.get("docker") is True
        or mock_builder.build.call_args[1].get("docker") is True
        or (len(mock_builder.build.call_args[0]) > 3 and mock_builder.build.call_args[0][3] is True)
    )
