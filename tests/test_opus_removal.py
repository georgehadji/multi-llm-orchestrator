...
class TestClaudeOpusRemovalFromEnum:
    ...
    def test_claude_opus_not_in_model_enum(self):
        ...
        assert "claude-opus-4-6" not in model_values, "CLAUDE_OPUS must be removed from Model enum"
...
class TestClaudeOpusRemovalFromCostTable:
    ...
    def test_claude_opus_not_in_cost_table(self):
        ...
        try:
            ...
            assert opus_model not in COST_TABLE, "CLAUDE_OPUS should not be in COST_TABLE"
    ...
    def test_all_routing_models_have_costs(self):
        ...
        for model in models_in_routing:
            assert model in COST_TABLE, f"Model {model} in ROUTING_TABLE but not in COST_TABLE"
...
class TestRoutingTableUpdates:
    ...
    def test_code_review_no_opus(self):
        ...
        try:
            ...
            assert (
                opus not in ROUTING_TABLE[TaskType.CODE_REVIEW]
    ...
    def test_code_review_has_minimax(self):
        ...
        assert (
            Model.MINIMAX_TEXT_01 in ROUTING_TABLE[TaskType.CODE_REVIEW]
    ...
    def test_reasoning_no_opus(self):
        ...
        try:
            opus = Model.CLAUDE_OPUS
            assert (
                opus not in ROUTING_TABLE[TaskType.REASONING]
    ...
    def test_reasoning_has_sonnet(self):
        ...
        assert (
            Model.CLAUDE_SONNET in ROUTING_TABLE[TaskType.REASONING]
    ...
    def test_writing_no_opus(self):
        ...
        try:
            opus = Model.CLAUDE_OPUS
            assert (
                opus not in ROUTING_TABLE[TaskType.WRITING]
    ...
    def test_writing_has_zai_glm_first(self):
        ...
        assert Model.GLM_4 in ROUTING_TABLE[TaskType.WRITING], "GLM_4 should be in WRITING routing"
        ...
        assert glm_index <= 1, "GLM_4 should be primary (#1) or early secondary (#2) in WRITING"

    def test_writing_has_sonnet_escalation(self):
        """WRITING should have CLAUDE_SONNET for quality escalation."""
        assert (
            Model.CLAUDE_SONNET in ROUTING_TABLE[TaskType.WRITING]
    ...
    def test_evaluate_no_opus(self):
        ...
        try:
            opus = Model.CLAUDE_OPUS
            assert (
                opus not in ROUTING_TABLE[TaskType.EVALUATE]
    ...
    def test_evaluate_has_minimax(self):
        ...
        assert (
            Model.MINIMAX_TEXT_01 in ROUTING_TABLE[TaskType.EVALUATE]
    ...
    def test_all_task_types_have_primary_model(self):
        ...
        for task_type, model_list in ROUTING_TABLE.items():
            assert len(model_list) > 0, f"{task_type} has no routing models"
            assert isinstance(model_list[0], Model), f"{task_type} first model is not a Model enum"

    def test_all_routing_models_exist(self):
        """Every model in ROUTING_TABLE must be a valid Model enum member."""
        for task_type, model_list in ROUTING_TABLE.items():
            for model in model_list:
                assert isinstance(model, Model), f"{task_type} contains invalid model: {model}"
...
class TestFallbackChainUpdates:
    ...
    def test_claude_opus_not_in_fallback_chain_as_key(self):
        ...
        try:
            opus = Model.CLAUDE_OPUS
            assert opus not in FALLBACK_CHAIN, "CLAUDE_OPUS should not be a key in FALLBACK_CHAIN"
        except AttributeError:
            pass
    ...
    def test_deepseek_reasoner_not_fallback_to_opus(self):
        ...
        try:
            ...
            assert reasoner_fallback != opus, "DEEPSEEK_REASONER should not fallback to CLAUDE_OPUS"
        except AttributeError:
            pass
    ...
    def test_deepseek_reasoner_fallback_to_sonnet(self):
        ...
        reasoner_fallback = FALLBACK_CHAIN.get(Model.DEEPSEEK_REASONER)
        assert (
            reasoner_fallback == Model.CLAUDE_SONNET
...
class TestModelConsistency:
    ...
    def test_no_broken_routing_references(self):
        ...
        for model in models_in_routing:
            assert model in COST_TABLE, f"{model} is in ROUTING_TABLE but not in COST_TABLE"
    ...
    def test_cost_table_completeness(self):
        ...
        for model, costs in COST_TABLE.items():
            ...
            assert costs["input"] > 0, f"{model} has invalid input cost: {costs['input']}"
            assert costs["output"] > 0, f"{model} has invalid output cost: {costs['output']}"
    ...
    def test_zai_glm_cheaper_than_opus_was(self):
        ...
        assert glm_cost["input"] < 5.0, "GLM_4 input cost should be much lower than Opus's $15"
        assert glm_cost["output"] < 20.0, "GLM_4 output cost should be much lower than Opus's $75"
